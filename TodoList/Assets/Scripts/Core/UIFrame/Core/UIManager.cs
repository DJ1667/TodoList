...
public class UIManager : MonoBehaviour
{
    ...
    {
        if (GetPanel<UIToastPanel>() == null)
        {
            OpenPanel<UIToastPanel>();
        }

        var panel = GetPanel<UIToastPanel>();
        panel.ShowToast(info);
    }
    ...
    public Vector2 GetWorldToUIPoint(Camera cam, Vector3 worldPos)
    {
        ...
        RectTransformUtility.ScreenPointToLocalPointInRectangle(GetUILayerRoot(UILayer.FullWindow), screenPos, UICamera, out uiPos);
        return uiPos;
    }
